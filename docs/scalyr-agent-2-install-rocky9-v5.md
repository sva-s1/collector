...
Installs **scalyr-agent-2** into a Python **venv** under `/opt`, with config + state + logs under **`/etc/scalyr-agent-2/`** (matching a working Collector layout).
...
# - nmap-ncat: provides `nc` for the tests that always worked
sudo dnf -y install \
...
Quick sanity (avoids “Cannot find command 'git'” + confirms `nc` exists):
...
## 3) Put secrets in an env file (recommended)
...
#
# Get your key from:
# https://community.sentinelone.com/s/article/000006763
#
...
#
# Optional: only needed if you want to override the server via env instead of JSON.
...
# Quiet "already running" noise on start (common when systemd already started it)
if [[ "${1:-}" == "start" ]]; then
  set +e
...
Put it on PATH for humans (and make `sudo scalyr ...` work on Rocky):
...
Create `/etc/systemd/system/scalyr-agent-2.service`:
...
> If you change `/etc/scalyr-agent-2/scalyr.env`, restart the service for changes to take effect.
...
### C) Send test syslog messages with netcat

Run these **on the agent host** (localhost), so you don't need to guess an IP.
...
## Find the test events in Singularity Data Lake (SDL)

1) Go to **Search**
2) Choose **XDR view** first
3) In the search box, use:

```text
logfile = '/etc/scalyr-agent-2/log/fortigate.log'
```

4) Time range: **Last 10 minutes**
5) If you don’t see results in XDR view, flip the view to **All Data**
...
Rocky `sudo` can ignore `/usr/local/bin`. Fix:
