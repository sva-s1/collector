...
# Scalyr Agent 2 (SentinelOne Collector) installer for Ubuntu 24.04.x LTS (Noble)
...
# ---------- OS guard (Ubuntu 24.04.x only) ----------
[[ -r /etc/os-release ]] || die "/etc/os-release not found"
# shellcheck disable=SC1091
. /etc/os-release

if [[ "${ID:-}" != "ubuntu" || "${VERSION_ID:-}" != "24.04" ]]; then
  die "Unsupported OS: ID='${ID:-}' VERSION_ID='${VERSION_ID:-}' VERSION='${VERSION:-}' (expected Ubuntu 24.04.x)"
fi
...
# Suppress pip "new release available" noise (we intentionally pin pip)
...
# ---------- step 4: venv + install ----------
step 4 "Creating venv + installing scalyr-agent-2 from Git tag ${TAG} (patched for Ubuntu pip/PEP517)..."

if [[ ! -x "${PY}" ]]; then
  python3 -m venv "${VENV}"
fi

"${PIP}" --disable-pip-version-check install -q --upgrade "pip==23.3.2" setuptools wheel >/dev/null 2>&1
"${PIP}" --disable-pip-version-check install -q six >/dev/null 2>&1

WORKDIR="$(mktemp -d)"
cleanup() { rm -rf "${WORKDIR}"; }
trap cleanup EXIT

git clone -q https://github.com/scalyr/scalyr-agent-2.git "${WORKDIR}/scalyr-agent-2"
cd "${WORKDIR}/scalyr-agent-2"
git checkout -q "${TAG}"
...
if "six" in txt:
    ...
    raise SystemExit(0)
import re
m = re.search(r'(?ms)^\[build-system\].*?^requires\s*=\s*\[(.*?)\]\s*$', txt)
if m and "\n" not in m.group(1):
    txt2 = re.sub(
        r'(?m)^(requires\s*=\s*\[)(.*?)(\]\s*)$',
        lambda mm: mm.group(1) + mm.group(2).rstrip() + (", " if mm.group(2).strip() else "") + '"six"' + mm.group(3),
        txt,
        count=1,
    )
...
for line in lines:
    if line.strip() == "[build-system]":
        ...
        out.append(line)
        continue
    if in_build and line.lstrip().startswith("requires") and "[" in line:
        in_requires = True
        out.append(line)
        continue
    if in_build and in_requires:
        if "]" in line:
            out.append('  "six",\n')
            out.append(line)
            in_requires = False
...
if count == 0:
    raise SystemExit("setup.py patch failed: import line not found")
p.write_text(txt, encoding="utf-8")
PY
fi

"${PIP}" --disable-pip-version-check install -q --no-use-pep517 . >/dev/null 2>&1
cd /
unset PYTHONPATH
