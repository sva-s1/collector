...
# ---------- OS guard (Rocky Linux 9 only) ----------
[[ -r /etc/os-release ]] || die "/etc/os-release not found"
# shellcheck disable=SC1091
. /etc/os-release

if [[ "${ID:-}" != "rocky" || "${VERSION_ID:-}" != "9"* ]]; then
  die "Unsupported OS: ID='${ID:-}' VERSION_ID='${VERSION_ID:-}' VERSION='${VERSION:-}' (expected Rocky Linux 9.x)"
fi
...
ensure_firewalld_514() {
  if command_exists firewall-cmd; then
    if systemctl is-active --quiet firewalld; then
      ...
      echo
      echo "firewalld is active. Ensured allow rules exist for 514/tcp and 514/udp."
      echo "firewalld rules (filtered):"
      firewall-cmd --list-ports 2>/dev/null | grep -E '(514/(tcp|udp))' || true
      echo
    else
      echo "firewalld is installed but not active; no firewall rules applied."
    fi
  else
    echo "firewalld not installed; no firewall rules applied."
  fi
}
...
detect_existing_install() {
  ...
  if command_exists rpm; then
    if rpm -q scalyr-agent-2 >/dev/null 2>&1; then
      found=1
      notes+=("rpm: scalyr-agent-2 package is installed")
    fi
  fi
...
prune_existing_install() {
  ...
  if command_exists rpm; then
    if rpm -q scalyr-agent-2 >/dev/null 2>&1; then
      dnf remove -y scalyr-agent-2 >/dev/null 2>&1 || true
...
command_exists ss || die "ss missing (dnf install iproute failed?)"
command_exists mountpoint || die "mountpoint missing (dnf install util-linux failed?)"
command_exists lsattr || die "lsattr missing (dnf install e2fsprogs failed?)"
command_exists lsof || die "lsof missing (dnf install lsof failed?)"
...
# ---------- step 2: packages ----------
step 2 "Installing OS packages via dnf (git, python, build deps, nc, tcpdump, jq)..."
...
command_exists jq  || die "jq missing (dnf install jq failed?)"
...
# ---------- step 8: wrapper + PATH ----------
step 8 "Writing wrapper ${WRAPPER} and enabling /usr/bin/scalyr (works with sudo secure_path)..."
...
step 9 "Writing systemd unit + enabling service (and opening firewall ports if firewalld is active)..."
